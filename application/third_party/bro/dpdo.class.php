			...
			if(is_null(self::$pdo)) {
				try{
					if(defined("DSN"))
						...
						$dsn="mysql:host=".BRO_HOST.";dbname=".BRO_DBNAME;

					//$pdo=new PDO($dsn, BRO_USER, BRO_PASS, array(PDO::ATTR_PERSISTENT=>true));
					$pdo=new PDO($dsn, BRO_USER, BRO_PASS);
			 ...
			 try{
				...
				if(isset($mem) && !$addcache){
					if($stmt->rowCount()>0){
						...
						log_message('debug', "清除表<b>{$this->tabName}</b>在Memcache中所有缓存!");
					}
				}
				...
				log_message('debug','[用时<font color="red">'.$ys.'</font>秒] - '.$memkey);
				return $return;
			}catch(PDOException $e){
				log_message('debug', "<font color='red'>SQL error: ".$e->getMessage().'</font>');
		...
		function setTable($tabName){
			...
			if(!file_exists($cachefile)){
				try{
					...
					log_message('debug', "<font color='red'>异常：".$e->getMessage().'</font>');
				}
			}else{
