	...
	class Dmysqli extends BRO_DB{
		...
		static function connect(){
			if(is_null(self::$mysqli)) {
				$mysqli=new mysqli(BRO_HOST, BRO_USER, BRO_PASS, BRO_DBNAME);
				if (mysqli_connect_errno()) {
					log_message('debug', "<font color='red'>连接失败: ".mysqli_connect_error().",请查看config.inc.php文件设置是否有误！</font>");
					return false;
				}else{
					self::$mysqli=$mysqli;
					return $mysqli;
		...
		function query($sql, $method,$data=array()){
			...
			$value=$this->escape_string_array($data);
			 ...
			 //如果SQL有误，则输出并直接返回退出
			 if(!$result){
				 log_message('debug', "<font color='red'>SQL ERROR: [{$mysqli->errno}] {$stmt->error}</font>");
				 log_message('debug', "请查看：<font color='#005500'>".$memkey.'</font>'); //debug
				 return;
			 }
			...
			if(isset($mem) && !$addcache){
				if($stmt->affected_rows > 0){ //有影响行数
					...
					log_message('debug', "清除表<b>{$this->tabName}</b>在Memcache中所有缓存!"); //debug
				}
			}
			...
			log_message('debug', '[用时<font color="red">'.$ys.'</font>秒] - '.$memkey,2); //debug
			return $returnv;
		}
		/**
		 * 获取多所有记录
		...
		function setTable($tabName){
			$cachefile=BRO_LIB."cache/".$tabName.".php";
			$this->tabName=BRO_TABPREFIX.$tabName; //加前缀的表名
			...
			log_message('debug', "表<b>{$this->tabName}</b>结构：".implode(",", $this->fieldList)); //debug
		}
    		/**
		* 事务开始
    		*/
		...
		//修复mysqli bind_param在5.3以后只能传引用参数(变量)
